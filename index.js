...
function nameChange(art) {
  art[8].name = "Vincent Van Gogh";
}  
nameChange(artists);  
...
function get20s(array) {
  let yearsArray = [];
  for (let i = 0; i < array.length; i++){ //try using 'i'
    if (array[i].years >= '1900') {
      yearsArray.push(array[i].name);
    }
...
function addArtist(newArt,current) {
  current.push(newArt);

  console.log(current);
}

addArtist(newArtists,artists);
